...
Horizontal scaling is a *xref:scalability.adoc[scalability]* technique that involves adding more servers to a system, rather than increasing the capacity of a single server (as in *xref:vertical-scaling.adoc[vertical scaling]*).

The aim is to increase xref:throughput.adoc[throughput] by distributing the load across multiple servers, rather than relying on a single server to handle all requests.
