...
Planning to mitigate single points of failure is a critical aspect of xref:system-design.adoc[system design]. A single point of failure (SPOF) is any part of a system that, if it fails, stops the entire system from functioning correctly.
...
Identifying and mitigating single points of failure increases the *xref:fault-tolerance.adoc[fault tolerance]* of a system.
...
* *High availability (HA)*: Designing systems to be highly available using active-active or active-passive xref:failover.adoc[failover] setups.

* *Fault tolerance*: Using xref:circuit-breaker.adoc[circuit breakers] and *[retry mechanisms]* to prevent failures from cascading.
