...
Constitutional methods are one layer of *xref:agent-harness.adoc[safety engineering]* for capable models. They do not guarantee safe behavior on their own. A constitution is only as good as the principles it contains and the training process that instills them. In practice, it is complemented by guardrails, monitoring, and careful design of the surrounding *xref:agent.adoc[agent]* harness.
