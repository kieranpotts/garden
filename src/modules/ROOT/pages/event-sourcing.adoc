...
Event sourcing is an *xref:architectural-pattern.adoc[architectural pattern]* in which every change in application state is captured as an *immutable event*. The state of an entity is determined by a sequence of events that are applied to it.
...
After processing the command, the system generates an event like `ItemsReceived +200`. This event is the factual record of the action. Events are immutable, meaning you cannot change them once they are recorded. The event gets added to the event log, preserving a precise sequence of actions like `ItemCreated`, `ItemsReceived +200`, and `ItemsShipped -3`.
