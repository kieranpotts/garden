...
GraphQL is a query language for APIs and a runtime for executing those queries, developed internally at Facebook in 2012 and open-sourced in 2015. It was designed to address the over-fetching and under-fetching problems common in *xref:rest.adoc[REST]* APIs, where response shapes are fixed by the server.
...
*xref:rest.adoc[REST]* remains preferable for simple CRUD APIs, public APIs where HTTP caching is important, or teams without GraphQL experience. *xref:grpc.adoc[gRPC]* is preferable for high-performance internal service-to-service communication.

== See also

* *xref:rest.adoc[Representational State Transfer (REST)]*: The architectural style GraphQL was designed to complement and, in some cases, replace.
