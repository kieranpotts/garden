...
A distributed system is a general term for any network of computers working as one coherent system. Distributed systems include *xref:cloud-computing.adoc[cloud computing]* services and *[peer-to-peer networks]*.
...
Examples of distributed software include applications like Hadoop (for *xref:big-data.adoc[big data]* processing), Kubernetes (for *xref:container-orchestration.adoc[container orchestration]*), and Cassandra (a *xref:distributed-databases.adoc[distributed database]*). Most *[web applications]* are also distributed programs. At the very least, web applications partition their processing between a *[client and server]*. The server-side application may be further partitioned into a *xref:service-oriented-architecture.adoc[service-oriented architecture]* such as *xref:microservices.adoc[microservices]*.

Distributed programming is the process of developing and maintaining distributed programs. Distributed computing is the field of *xref:computer-science.adoc[computer science]* that studies distributed systems.
...
Most large-scale systems are naturally distributed. That's because *xref:monolith.adoc[monoliths]*, as they grow in scope and load, become ever harder to maintain, develop, operate, and *[scale]*. So the natural tendency is toward breaking down monoliths into lots of smaller and simpler components that can each be developed, deployed, operated, scaled, and optimized independently. This process is known as *xref:decomposition.adoc[decomposition]*.
...
Although individual units within a distributed system may be simpler and more *[resilient]* due to increased *xref:isolation.adoc[isolation]*, overall a distributed system will always have more *[points of failure]* and a higher degree of *xref:complexity.adoc[accidental complexity]* than single-node systems with comparable functionality. Distributed software is inherently *[brittle]* because you're introducing internal network calls, which are subject to *xref:fault-tolerance.adoc[network failures]* and greater operational constraints like *xref:latency.adoc[latency]* and *[bandwidth]*.

Reasoning about *xref:concurrency.adoc[concurrency]*, managing *xref:consistency.adoc[data consistency]*, and designing *xref:monitoring.adoc[monitoring]* solutions, are other concerns that are particularly challenging in distributed software design.

_Distributed programming is hard._ The main challenges in designing distributed software are:

* *Fault tolerance*: *xref:fault-tolerance.adoc[Fault tolerance]* is a key *xref:quality-attributes.adoc[quality attribute]* of distributed systems, and it needs to be treated as a first-class concern in the *xref:system-design.adoc[system design]*. Distributed software is subject to a wide range of potential failure modes, including network failures, delayed messages, and node crashes. The principle of *[designing for failure]* means designing each service in a way that it can tolerate failures in dependent services. For example, if an email service goes offline, emails should be queued for delivery and sent later when the email service comes back online. *[Asynchronous communication patterns]* improve the fault tolerance of distributed systems by reducing *temporal coupling* (where the availability of one service is dependent on the immediate availability of another). Design patterns such as *xref:circuit-breaker.adoc[circuit breakers]* and *xref:bulkhead.adoc[bulkheads]* also help to improve the fault tolerance of distributed systems by isolating failures, so preventing failures in one component from cascading into other components. *xref:disaster-recovery.adoc[Disaster recovery]* planning is also important. It is the process of designing services to recover quickly, in an automated way, from failures.
...
* *Concurrency and consistency*: Because distributed systems generally use asynchronous communication between services, managing *xref:concurrency.adoc[concurrency]* and ensuring *xref:consistency.adoc[eventual consistency]* become key design concerns. *xref:cap-theorem.adoc[CAP theorem]* is relevant in distributed systems that *[replicate]* data between nodes.

* *Observability*: *xref:monitoring.adoc[Monitoring]*, *xref:logging.adoc[logging]*, and *[debugging]* all become much more complex in distributed systems. *xref:observability.adoc[Observability]* and especially *xref:tracing.adoc[distributed tracing]* tools are specifically designed to address these challenges.

* *xref:testing.adoc[Testing]*, too, is much more challenging in distributed systems. It is harder, for example, to do proper integration tests to validate a whole system, compared to monolithic systems where all the components run in the same process. There tends to be increased reliance upon service mocks, which reduces confidence in correctness of the tests.
...
For these reasons, distributed computing should be avoided until it is required to solve specific problems, usually something to do with *xref:scalability.adoc[scalability]*. Better to start with a *xref:modular-monolith.adoc[modular monolith]*, with scalability planned into the design from the start, and extract independent services incrementally.
