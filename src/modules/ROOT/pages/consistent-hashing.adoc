...
Consistent hashing is a special kind of xref:hashing.adoc[*hashing*] technique that allows for
efficient xref:sharding.adoc[*sharding*] of data across a cluster of nodes, and so underpins the
xref:scalability.adoc[*scalability*] of distributed data stores by letting a cluster grow one node
at a time without rehashing the whole keyspace.
...
nodes located around a hash range. Each data item is also hashed, using the same hash function, to
give it a location on the same hash range. The node that is responsible for storing a data item is
the nearest to it along the hash range.
...
xref:nosql-databases.adoc[*NoSQL databases*], including Cassandra and Riak, which place keys on a
hash ring to distribute load evenly across the cluster.
