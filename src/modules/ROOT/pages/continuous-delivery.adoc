...
Continuous delivery is _not_ the same as *xref:continuous-deployment.adoc[continuous deployment]*. Continuous delivery means the software is _not automatically deployed_ to production systems, but just that you could – in theory – manually trigger an automated production deployment at a moment's notice.
