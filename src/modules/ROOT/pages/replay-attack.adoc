...
* *xref:nonce.adoc[nonces (numbers used once)]*, unique random values embedded in each request that the server validates and discards after use;
...
Modern protocols such as TLS incorporate replay protection through session-specific keys and sequence tracking, and short-lived tokens (such as *xref:jwt.adoc[JWTs]* with tight expiry times) limit the window of opportunity for a captured credential to be reused.
