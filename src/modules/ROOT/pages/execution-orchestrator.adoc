...
This is in contrast to non-mediated patterns like *xref:pipe-and-filter.adoc[pipe-and-filter]*, where data flows through workers connected directly by pipes, and *[choreography]*, where workers react to events without a central coordinator.
...
The trade-off is that the orchestrator can become a *xref:single-point-of-failure.adoc[single point of failure]* and a performance bottleneck. This can be mitigated by making the orchestrator itself highly available and durable.
