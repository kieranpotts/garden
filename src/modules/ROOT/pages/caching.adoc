...
The primary goal of caching is to improve system *xref:performance.adoc[performance]*. Other benefits include higher *xref:availability.adoc[availability]*, and *xref:scalability.adoc[scalability]*.

These benefits are achieved by reducing *xref:latency.adoc[latency]* (for example by using technologies that are more highly optimized for fast data retrieval than the main data store), and increasing *xref:throughput.adoc[throughput]* (for example by offloading some requests from the main data store).
...
* *xref:key-value-cache.adoc[Key-value cache (KV cache)]*: A specialized cache used by transformer-based language models to store intermediate attention states during inference, avoiding repeated computation for previously generated tokens.
...
Caching comes with its own sets of challenges. The main difficulty is maintaining data *xref:consistency.adoc[consistency]*. When data changes in the source-of-truth, the cached copies become stale. There are various strategies that can be implemented to maintain consistency between sources and caches. Caching strategies include:
...
* *Cache penetration*: This happens when a requested key does not exist in either the cache or the database, and therefore the application cannot retrieve the relevant data from any source. One possible mitigation strategy is to use a *xref:bloom-filter.adoc[bloom filter]* to check for the key's existence. If the key does not exist, we can avoid querying the database.
...
* *Cache crash*: This happens when a cache is overwhelmed by requests, and it crashes. This can be mitigated by using a *xref:circuit-breaker.adoc[circuit breaker]* to temporarily stop requests to the cache, giving it time to recover. Risks of cache crashes can be reduced by improving availability through cache *xref:replication.adoc[replication]*.
