...
A distributed transaction is a type of *xref:transaction.adoc[transaction]* that ensures that all operations across a xref:distributed-system.adoc[distributed system], such as xref:microservices.adoc[microservices], are completed successfully, or none at all, maintaining the xref:data-integrity.adoc[data integrity] of the overall system.
