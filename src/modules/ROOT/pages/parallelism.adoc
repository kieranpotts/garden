...
There are also risks associated with parallelism, such as the risk that simultaneous changes to the same data may corrupt it. Techniques such as *xref:locking.adoc[locking]* can be used to mitigate these risks.
...
****
Parallelism is a separate concept to *xref:concurrency.adoc[concurrency]*. A parallel program uses multiple CPU cores, each performing a task independently. A concurrent program uses a single CPU core but switches between tasks (usually represented by threads) to make the most efficient use of CPU time.
