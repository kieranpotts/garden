...
Layers 4 (transport) and 7 (application) are the most relevant for software developers. Layer 4 determines whether data is delivered reliably (TCP) or with minimal overhead (UDP), and is the layer at which *xref:load-balancer.adoc[Layer 4 load balancers]* operate. Layer 7 is where application protocols live and where *xref:load-balancer.adoc[Layer 7 load balancers]*, *xref:reverse-proxy.adoc[reverse proxies]*, and *xref:api-gateway.adoc[API gateways]* make routing decisions based on request content.
