...
Consistency is a key challenge in *xref:distributed-system.adoc[distributed software]* design. It is the property that all nodes in a distributed software system agree on the state of the overall system.

As soon as you *[replicate]* data across multiple nodes – which you do to improve *xref:fault-tolerance.adoc[resilience]* and *xref:availability.adoc[availability]* – you introduce the possibility that the data on different nodes will be out of sync. You have to deal with the problem of how to maintain consistency.

A key idea is *xref:eventual-consistency.adoc[eventual consistency]*. The aim is for data across the whole system to eventually be consistent, but it is not guaranteed to be consistent at any particular point in time.
