...
In *xref:domain-driven-design.adoc[domain-driven design]* (DDD), each subdomain maps to a *xref:domain-model.adoc[domain model]* — a structured set of abstractions capturing the relevant concepts and their relationships. These models are expressed in a *xref:ubiquitous-language.adoc[ubiquitous language]*: a shared vocabulary agreed upon by developers and *xref:domain-experts.adoc[domain experts]* (subject-matter experts from the business) that is used consistently in conversation, documentation, and code.
