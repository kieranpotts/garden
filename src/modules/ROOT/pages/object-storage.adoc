...
Also known as blob storage (or binary large object storage), object storage is a technology that stores and manages data as discrete units called objects. Rather than organizing data into a hierarchy of files and folders (as *xref:file-storage.adoc[file storage]* does), object storage uses a flat namespace: all objects live in a single container called a bucket, each identified by a unique key. Objects can be anything – images, videos, audio files, backups, log files, machine learning datasets.
...
Unlike file storage (NFS/SMB) or *xref:block-storage.adoc[block storage]*, object storage is not mounted as a filesystem. Data is accessed programmatically via an API (the de facto standard being Amazon S3's API). This design removes the need for a traditional file-system layer, which is what allows object storage to scale to virtually unlimited capacity across multiple physical devices, data centres, and regions.
