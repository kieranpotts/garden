...
Also known as *xref:blob-storage.adoc[blob storage]*, object storage is a technology that stores and manages data as discrete units called *objects*. Rather than organising data into a hierarchy of files and folders (as *xref:file-storage.adoc[file storage]* does), object storage uses a flat namespace: all objects live in a single container called a *bucket*, each identified by a unique key. Objects can be anything — images, videos, audio files, backups, log files, machine learning datasets.
