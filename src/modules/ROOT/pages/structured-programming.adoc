...
Structured programming also laid the groundwork for *xref:stepwise-refinement.adoc[stepwise refinement]* and later paradigms. *xref:object-oriented-programming.adoc[Object-oriented programming]*, for example, built on its procedural foundations while adding mechanisms for organizing data alongside behavior.
