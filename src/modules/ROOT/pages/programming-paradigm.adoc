...
* *xref:concatenative-language.adoc[Concatenative programming]*: A concatenative programming language is a functional programming paradigm in which programs are built by composing (concatenating) functions.
...
* *xref:stack-language.adoc[Stack language]*: A stack language is one in which the execution model involves pushing operands onto a stack, from which operators consume or produce stack values.
