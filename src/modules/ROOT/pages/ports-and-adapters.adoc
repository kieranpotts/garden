...
Ports-and-adapters is a *xref:design-patterns.adoc[design pattern]* that is closely associated with *xref:hexagonal-architecture.adoc[hexagonal architecture]*.
...
There is some synergy between hexagonal architecture and *xref:domain-driven-design.adoc[domain-driven design]*. The hexagons – which could represent services in distributed software or modules in monoliths – could model subdomains. And the *input ports* of the ports-and-adapters pattern map quite nicely to the concept of *commands* in DDD. However, DDD tends to emphasize the modeling of data and persistence layers (entities, repositories) whereas ports-and-adapters emphasizes stateless outputs.
