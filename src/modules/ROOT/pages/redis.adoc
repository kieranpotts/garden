...
Redis (Remote Dictionary Server) is an open-source, *xref:in-memory-databases.adoc[in-memory]* key-value store, widely used as a cache, session store, message broker, and real-time data structure server. A single Redis server can handle over 100,000 requests per second. Despite being single-threaded, it consistently delivers sub-millisecond latency at high throughput.
