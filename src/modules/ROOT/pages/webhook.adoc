...
A webhook is a mechanism for one system to send real-time notifications to another system over *xref:http.adoc[HTTP]*. Instead of polling for updates, a consumer registers a callback URL with a producer, and the producer sends an HTTP POST request to that URL whenever a relevant event occurs.
