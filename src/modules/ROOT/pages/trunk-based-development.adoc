...
Trunk-based development (TBD) is a *xref:version-control.adoc[source control]* branching model that encourages small increments of work to be integrated regularly into a shared mainline branch – commonly called `trunk`, `master`, or `main`.

Trunk-based development attempts to avoid *xref:big-bang.adoc[big-bang]* merges, and the increased risks of merge conflicts that come with those. This is done by adhering to the following principles:
...
Trunk-based development enables *xref:continuous-integration.adoc[continuous integration]* and (by extension) *xref:continuous-delivery.adoc[continuous delivery]*. It is supported by techniques such as *xref:feature-flags.adoc[feature flags]*, which replace the need for long-lived side branches to support feature development. Instead of keeping unfinished features isolated from other developers using side branches, you keep unfinished features isolated from your end users by disabling the code in production behind feature toggles.
