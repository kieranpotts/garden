...
The outcome of the strategic design phase is a comprehensive *xref:domain-model.adoc[model]* of the software's *xref:domain.adoc[domain]* or *[problem space]*. Domain models may be expressed in documentation, code, tests, or visually (eg. using UML or similar graphical notations), or a combination of all these. DDD is not prescriptive about the choice of modeling artifacts. The only criteria is that a consistent language – an *[ontology]* – is used throughout the model.
