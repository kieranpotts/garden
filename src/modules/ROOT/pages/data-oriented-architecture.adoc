...
Thus, a DOA is a hybrid of *xref:monolith.adoc[monolithic]* and *xref:service-oriented-architecture.adoc[service-oriented]* (and especially *xref:microservices.adoc[microservice]*) designs.
...
One of the big advantages of this design pattern is the ease with which data *xref:consistency.adoc[consistency]* can be maintained across the system. Component-to-component interaction is also minimized, favoring data sharing through the central store, potentially reducing overall latency in the system.
