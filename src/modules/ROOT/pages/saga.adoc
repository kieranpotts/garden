...
The saga pattern – whether choreographed or coordinated – offers higher *[resilience]* and xref:scalability.adoc[scalability] compared to traditional phased-commit transactions, as it avoids blocking issues which are associated with two-phase commit protocols especially. However, the trade-off is that sagas can be more complex to implement and manage, and xref:consistency.adoc[consistency] is traded for xref:availability.adoc[availability].

When you're dealing with distributed sagas, especially choreographed ones, xref:observability.adoc[observability] – and *[distributed tracing]* in particular – becomes crucial.
