...
As organizations build more *xref:ai-agent.adoc[agentic]* systems, they increasingly need agents built by different teams — and on different platforms — to work together. Without a common protocol, every agent-to-agent integration is bespoke.
...
A2A is complementary to the *xref:model-context-protocol.adoc[Model Context Protocol (MCP)]*, not a competitor to it. The two solve different problems. MCP connects an agent to its tools and data sources, while A2A connects agents to other agents. An *xref:agent harness[ai-agent-harness-engineering.adoc]* may combine both — MCP to give each agent its capabilities, and A2A to let those agents collaborate.
