...
The *Transmission Control Protocol (TCP)* is a *xref:internet-protocol.adoc[Layer 4]* transport protocol that provides *reliable*, *ordered*, and *connection-oriented* delivery of data between two hosts over an *xref:internet-protocol.adoc[IP]* network. It is one half of the TCP/IP suite that underpins most internet traffic, used by *xref:http.adoc[HTTP/HTTPS]*, email (SMTP), and file transfer (FTP).
...
TCP's reliability comes at a cost: the handshake, acknowledgements, and retransmissions all add latency and overhead. For applications where timeliness matters more than guaranteed delivery — real-time audio/video, gaming, DNS lookups — *xref:udp.adoc[UDP]* is preferred.
