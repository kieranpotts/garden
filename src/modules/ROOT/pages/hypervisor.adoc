...
A hypervisor, also known as a *virtual machine monitor*, is a technology that makes *xref:virtualization.adoc[virtualization]* possible.

A hypervisor manages access to the physical hardware by several operating systems at once. Each operating system runs in its own *xref:virtual-machine.adoc[virtual machine]*, managed by the hypervisor. Each OS is unaware of neighboring operating systems running on the same hardware.
