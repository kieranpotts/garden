...
A content delivery network (CDN) is a type of *xref:caching.adoc[cache]* that stores copies of digital files in multiple geographic locations. The aim is to reduce *[latency]*, improve *[performance]*, and increase *[availability]* by serving files from the nearest servers to the end user.
