...
Container orchestration, also known as *container management*, refers to the automated management of *xref:containerization.adoc[containerized]* applications. Container orchestration systems manage the lifecycle of *xref:container.adoc[containers]*, from deployment to destruction.
...
* *Load balancing*: Distributing *xref:load.adoc[workloads]* evenly across containers, to optimize resource use and performance.
...
The most popular container orchestration system is *xref:kubernetes.adoc[Kubernetes]*. Others include:
