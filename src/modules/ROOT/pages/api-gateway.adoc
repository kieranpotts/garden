...
Since an API gateway is a potential *[single point of failure]*, it is typically deployed in a *xref:load-balancing.adoc[load-balanced]* configuration, with traffic distributed between multiple instances. Additional *xref:load-balancer.adoc[load balancers]* may sit behind the API gateway to further distribute traffic between multiple instances of the back-end services.
