...
Either side sends a `close` frame (opcode `0x8`), optionally including a status code and a reason string. The receiver echoes back a `close` frame and both sides then close the underlying TCP connection. The status code 1000 means normal closure. The status code 1001 means the endpoint is going away (e.g. server restart or browser tab close).
...
Unlike xref:server-sent-events.adoc[SSE], the browser provides no automatic reconnection for WebSockets. Applications must implement their own reconnection logic, typically with exponential backoff.
...
* xref:server-sent-events.adoc[Server-Sent Events (SSE)] — A simpler, HTTP-native alternative for one-directional server push.
