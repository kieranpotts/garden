...
Reactive programming is a *declarative programming* paradigm that first emerged in the earliest implementation of graphical user interfaces, but has since been applied to many other types of software systems, including *xref:distributed-system.adoc[distributed software]*.

Reactive programming is based on the *xref:asynchronous-processing.adoc[asynchronous processing]* of *xref:event-stream.adoc[event streams]*. In a reactive system, the program reacts to changes in its environment, such as user input, messages from other systems, or changes in the state of the system itself. These changes are represented as events, which are processed by the system asynchronously.
...
The solution was to decouple the event handling from the GUI update. This is done by introducing an *event queue*, which is processed asynchronously. A producer thread handling user input pushes events to the queue, and a consumer thread takes those events and processes them. This is an implementation of the *xref:observer-pattern.adoc[observer pattern]*.
