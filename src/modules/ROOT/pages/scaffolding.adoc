...
The metaphor is drawn from construction, where scaffolding provides the temporary structure that enables workers to build a permanent one. Software scaffolding serves the same role. Stubs, mock data, xref:test-harness.adoc[test harnesses], and throwaway driver programs that allow components to be developed and exercised in isolation before the surrounding system exists. Once the real components are in place, the scaffolding is removed.
