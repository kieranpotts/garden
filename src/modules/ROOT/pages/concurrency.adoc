...
Concurrency gives the _illusion_ of *xref:parallelism.adoc[parallelism]*, but it is not the same thing. In *xref:parallelism.adoc[parallel programming]*, a program uses multiple CPU cores, each performing a task entirely independently of the others. Parallel programming is true *multi-tasking*, meaning tasks are literally processed "at the same time". Concurrency gives the impression of multi-tasking, but really it is *multi-threading*.
