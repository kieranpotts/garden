...
from being overwhelmed by retries. Circuit breakers are also commonly paired with *xref:bulkhead.adoc[bulkheads]*, which partition the resources used to call a dependency so that one failing call cannot exhaust the threads or connections available to other callers.
