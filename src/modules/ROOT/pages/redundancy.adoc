...
In the context of a *xref:distributed-system.adoc[distributed software]*, redundancy refers to the duplication of critical components or functions of the system. Adding redundancy reduces single points of failure, thus increasing *xref:fault-tolerance.adoc[reliability]* and *xref:availability.adoc[availability]*. If one part of the system fails, another can take over its tasks, minimizing *xref:downtime.adoc[downtime]* and preventing data loss.
...
Redundancy is an important *xref:quality-attributes.adoc[quality attribute]* in distributed software that requires high *xref:availability.adoc[availability]* and *xref:fault-tolerance.adoc[resilience]*. Examples of systems that have high levels of redundancy include data centres and the cloud services that are served from them.

*xref:isolation.adoc[Isolation]* is a complementary design principle. This is about limiting the impact of failures so that they don't affect the entire system. For example, if the primary node for a service fails, and if its failovers fail too, the rest of the system should continue to operate (albeit with reduced functionality).
