...
SSE is a W3C standard, natively supported in all modern browsers through the `EventSource` API. Unlike *xref:websockets.adoc[WebSockets]*, which are bidirectional, SSE is unidirectional: data flows only from server to client.
