...
ECMAScript is the formal specification that standardizes the scripting language on which xref:javascript.adoc[JavaScript] is based. It is maintained by ECMA International, a standards body, through its *TC39* technical committee.

The ECMAScript specification defines the core syntax, semantics, and standard library for the language. JavaScript is the most widely known implementation of ECMAScript. xref:typescript.adoc[TypeScript] is a strict superset of JavaScript that also conforms to the ECMAScript specification, while adding a static type layer on top.
