...
The infrastructure that wraps the loop — providing tools, managing the context window, enforcing guardrails — is the *xref:agent-harness.adoc[harness]*. Running many such loops unattended, on a recurring cadence rather than a single session, is the concern of *xref:loop-engineering.adoc[loop engineering]*.
