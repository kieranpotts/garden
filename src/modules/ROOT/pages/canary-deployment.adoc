...
A canary deployment is a *xref:deployment-strategies.adoc[deployment strategy]* in which a new version of software is initially released to a small subset of users before being rolled out to everyone.
...
Routing is typically implemented at the *xref:load-balancer.adoc[load balancer]* level, using weighted traffic rules. More sophisticated canary deployments can target specific user segments – for example, internal users, beta testers, or users in a specific geographic region – rather than random samples.
