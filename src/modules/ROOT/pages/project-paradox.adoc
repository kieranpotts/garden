...
decisions we will need to make once construction starts. Approaches that invest
heavily in this up-front analysis overlap with
xref:big-design-up-front.adoc[*big design up-front*].

Another approach, commonly referred to as being consistent with
xref:agile.adoc[*agile*] ways of working, is to design systems with a strong
bias toward support for changeability, ie. xref:evolvability.adoc[*evolvability*].

xref:galls-law.adoc[*Gall's law*] makes the same point from the opposite
direction: a complex system designed from scratch never works, so it is safer
to grow one from a simple working core than to try to make every early decision
correctly.
