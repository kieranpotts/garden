...
Our industry's current bias towards *xref:agile.adoc[agile]* software delivery practices has led to a preference for use of lightweight diagramming tools to create visual representations of *xref:system-design.adoc[system designs]*, over the production of detailed *xref:modeling.adoc[models]*.
...
* https://www.gliffy.com/[Gliffy] — Easy-to-use web-based diagramming application, supporting collaborative editing and a large number of diagram types.
