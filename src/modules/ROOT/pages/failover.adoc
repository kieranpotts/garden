...
Failover improves the availability and *xref:scalability.adoc[scalability]* of a
system. It can help to remove *single points of failure* and ensure that a
system remains operational even if some of its components fail.
...
Failover requires xref:redundancy.adoc[redundancy], which is about having backup instances that can step in without user impact. To avoid downtime, you keep standby services online.
...
The choice between the two affects everything: *latency*, *cost*, *availability*, and *operational complexity*.
...
But failover isn't instant. Depending on the detection and promotion logic (manual, DNS TTL, heartbeat monitoring, etc.) recovery can take seconds to minutes. During that time, the service is unavailable.
...
This setup gives you the throughput and fault tolerance of active-active, but with lower cost and simpler coordination, because not every node is participating at all times.
