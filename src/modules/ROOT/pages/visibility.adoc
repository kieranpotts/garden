...
Visibility is a xref:quality-attributes.adoc[quality attribute] of a system. Broadly, visibility refers to the ability to understand and analyze a system's behavior, performance, and health when it is running in production.

Visibility is implemented using xref:monitoring.adoc[monitoring] and xref:observability.adoc[observability] strategies. Monitoring and observability are closely related, but serve different purposes:

* Monitoring is about tracking xref:metrics.adoc[metrics] against predefined thresholds, and triggering xref:alerting.adoc[alerts] when metrics fall outside of the acceptable ranges.

* Observability is about using various xref:telemetry.adoc[telemetry] data sources to gain insights into a system's behavior, typically to diagnose issues and respond to incidents.
