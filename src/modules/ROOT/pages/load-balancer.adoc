...
A load balancer is a network device or software application that distributes incoming network traffic across multiple backend servers or other resources. This is a technique for *[horizontal scaling]*, and is widely used in *xref:distributed-system.adoc[distributed software]* to improve capacity, availability, and *xref:fault-tolerance.adoc[fault tolerance]*.
