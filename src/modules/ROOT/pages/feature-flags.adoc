...
Feature flags are an essential component of the technical *[architecture]* of a software system where advanced *[agile development methodologies]* and *xref:devops.adoc[DevOps]* practices are desirable. Feature flags support *xref:trunk-based-development.adoc[trunk-based development]*, *xref:continuous-delivery.adoc[continuous delivery]* and *xref:continuous-deployment.adoc[continuous deployment]* (which depend on trunk-based development), *[canary releases]* and *[phased rollouts]*, and *xref:ab-testing.adoc[A/B testing]*.
...
Feature flag systems should not conflate these different concerns. The following is an example of an *xref:anti-pattern.adoc[anti-pattern]* in which feature flag logic is used in place of an authorization system to restrict access to certain features based on user roles or permissions. This design can lead to *xref:technical-debt.adoc[technical debt]* and reduced *xref:evolvability.adoc[evolvability]* of the software system.
