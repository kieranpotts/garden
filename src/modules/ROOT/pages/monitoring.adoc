...
Monitoring is the process of collecting, analyzing, and using data to track the xref:performance.adoc[performance] and health of a system.
...
Monitoring is closely associated with *xref:observability.adoc[observability]*. The same tools and data can be used for both, but the two concepts are distinct. Monitoring is a _reactive_ process, reacting to events and changes in state based on pre-configured rules. Observability is a _proactive_ process, focused instead on analyzing a systems behavior and performance after the fact.
