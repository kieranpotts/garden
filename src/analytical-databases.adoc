...
Analytical databases are used for *Online Analytics Processing (OLAP)*. *[Data warehouses]* may be used for this purpose, with the source data coming from the Online Transaction Processing (OLTP) done by *link:./transactional-databases.adoc[transactional databases]*, or other sources.
