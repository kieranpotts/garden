...
LLMs are a category of *link:./generative-ai.adoc[generative AI]* tools. LLMs are increasingly deployed in a wide variety of contexts. *link:./src/ai-assistant.adoc[AI assistants]* are perhaps the best-known category of application software built on top of LLMs, but many other use cases are being found for LLMs.
...
Weights refer to a model's learned parameters – the billion of numbers that encode everything the model learned during training. Weights determine how a model transforms inputs into output.
...
As of April 2026, the following open-weight models are highly regarding for the computer programming abilities. The small models (≤ 7B) are suitable for running on laptops and CPU-only setups, while the larger models (eg. CodeLlama 34B, LLaMA 2 Code 70B) provide deeper reasoning but need GPUs with ≥ 24GB VRAM.
