...
* The https://www.minimax.io/[MiniMax] models from MiniMax AI.

*Gemma* is Google's open-weight counterpart to the proprietary Gemini family, optimized to run efficiently on consumer hardware.

*Mistral AI*, a French company, produces both open-weight models and proprietary API offerings; their Mixtral series helped popularize the Mixture-of-Experts (MoE) architecture in open models.

*MiniMax* also develops both closed-weight and open-weight models. MiniMax M2.5 is well-regarded for its performance, though it lags in intelligence compared to something like Qwen 3.5.

*DeepSeek* made significant waves in early 2025 when DeepSeek-R1 demonstrated near-frontier reasoning performance at a fraction of the training compute cost that had previously been assumed necessary.

The *Qwen* models are available in a wide range of sizes, so there'll always be one variant that you can run on your hardware.

Nvidia is also entering the open-weight model space, with the https://research.nvidia.com/labs/nemotron/Nemotron-3/[*Nemotron 3*] family of models.
