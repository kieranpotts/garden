...
across multiple nodes, reducing *[load]* and *link:./fault-tolerance.adoc[risk]* on any single node.
This improves system *link:./performance.adoc[performance]*, *link:./availability.adoc[availability]*,
and *link:./scalability.adoc[scalability]*.

Trade-offs include increased *:./complexity.adoc[complexity]* in the system design, new challenges
for joining data across shards, and the potential for *hot spots* that can be solved only through
tricky *data rebalancing*.

Database sharding is commonly used in situations where a single database can no longer be
sufficiently *link:./vertical-scaling.adoc[scaled vertically]*. Sharding is one of the two main
methods for scaling databases. The other is *link:./replication.adoc[replication]*.
