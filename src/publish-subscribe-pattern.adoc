...
Publish-subscribe is a *link:./design-patterns.adoc[design pattern]* in which a centralized component acts as an intermediary between two distinct groups of software components: *publishers* and *subscribers*. Publishers send messages (typically in the form of events) to the central message *broker*, and the broker is responsible for delivering the messages to the subscribers who are interested in them.
