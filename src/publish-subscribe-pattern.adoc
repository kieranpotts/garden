...
*[dependency inversion principle]*. The high-level components (publishers) are not dependent upon
the low-level components (consumers), and vice versa. Instead, both types of components depend on
an abstraction (the event messages themselves). This means that components (or services) can be
more easily added, changed, replaced, without impacting the rest of the system.
