...
The producers and consumers of events should have no direct communication with each other. Instead, all communication in an event-driven system should be done *link:./asynchronous-communication.adoc[asynchronously]* via some kind of message channel or broker.
