...
There is some synergy between hexagonal architecture and *[domain-driven design]*. The hexagons – which could represent services in a distributed system or modules in a monolith - could model subdomains. And the *input ports* of the ports-and-adapters pattern map quite nicely to the concept of *commands* in DDD. However, DDD tends to emphasize the modeling of data and persistence layers (entities, repositories) whereas ports-and-adapters emphasizes stateless outputs.
