...
name: uproot
description: >-
  Drop one redundant entry whose topic another entry already covers,
  repointing every reference to the covering entry and removing its index and
  nav listings. Use this skill when the user says something like "uproot
  acid.adoc — it's already covered by acid-principles.adoc", "this entry is
  redundant, drop it", or asks whether one entry is made redundant by another.
  Do not use it when each entry carries distinct content, which calls for a
  merge instead.
compatibility: >-
...
- Three or more entries all cover the same topic. Uproot one target per run,
  ...
  the deletions. You'll produce a bigger diff than the user expects.
